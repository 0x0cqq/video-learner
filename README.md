...
转换结束自动显示 Token meter 和 Estimated 费用，并保存 `usage.json`，包括重试用量、未知部分和所用单价。默认价格来自用户提供的 Qwen 报价及 DeepSeek 公开价格快照，可通过 TOML 覆盖；见[计费配置](docs/usage.md#token-meter-与估算费用)。
