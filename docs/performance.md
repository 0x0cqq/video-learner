...
同机四课 CUDA 整段识别与阶段计时见[整课验证](validation.md#四个完整课时与学习者审阅)。本轮通过保存转写、候选图、原始响应及单章草稿复用内容；阅读校订导出使用 tools/export_review.py，无额外模型费用。
