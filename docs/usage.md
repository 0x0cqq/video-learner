...
`--config` 指定 TOML；优先级为命令行 > TOML > 默认值。修订时先继承指定版本的转换配置，再应用 TOML 和命令行覆盖；凭据文件路径未保存在基线中，需重新指定或使用环境变量。可参考 [config.example.toml](../config.example.toml)。明确要求额外解释时使用 `--instruction` 并加 `--allow-ai-additions`；生成内容会单独标为 AI 补充解释。

以下 PowerShell 示例均从项目根目录执行，`D:\media\` 中的素材目录作为占位路径；替换为自己的视频或单课时缓存，并将输出放在素材目录之外。
...
```powershell
uv run video-learner convert "D:\media\programming" --start 00:45:00 --end 00:55:00 --profile programming --asr-secret ".\secrets\aliyun.secret" --secret ".\secrets\deepseek.secret" --output ".\output\sample"
```

时间可用秒数或 `HH:MM:SS`，最多六位小数，按 `[start,end)` 处理。省略范围转换整课。输出目录必须与源目录不重叠。默认拒绝已存在的目录；加 `-f` / `--force` 会在预检通过并取得目录锁后删除整个原输出，再从头转换。旧版本、手改、日志均不保留，转换失败不会恢复旧结果。强制覆盖拒绝磁盘根目录、当前工作目录及其上级、符号链接或目录联接，也拒绝将本次字幕、凭据或本地模型一并删除。
...
`--subtitle` 可指定 UTF-8 SRT/VTT 替代 ASR。字幕需时间合法，且至少一条与所选范围相交；覆盖比例写入工作记录，缺失区间与同步仍需人工核对。发现但未显式选择的字幕不会自动覆盖语音识别。
...
```powershell
uv run video-learner convert "D:\media\math" --start 00:10:00 --end 00:20:00 --profile math --asr-secret ".\secrets\aliyun.secret" --secret ".\secrets\deepseek.secret" --output ".\output\my-qwen-notes"
```
...
```powershell
uv run --extra local-asr video-learner convert "D:\media\schopenhauer" --start 00:05:00 --end 00:06:00 --asr-backend local --asr-device cpu --jobs 2 --secret secrets/deepseek.secret --output output/schopenhauer
```
...
uv run --no-sync video-learner convert "D:\media\schopenhauer" --start 00:05:00 --end 00:06:00 --asr-backend local --asr-device cuda --secret secrets/deepseek.secret --output output/schopenhauer-cuda
```
...
```powershell
uv run video-learner convert "D:\media\math" --start 00:14:30 --end 00:15:30 --profile math --provider qwen --asr-secret ".\secrets\aliyun.secret" --secret ".\secrets\aliyun.secret" --output ".\output\my-qwen38-notes"
```
...
```powershell
uv run video-learner revise ".\output\sample" --base r001 --block fig-001-002 --instruction "压缩图注为一句话" --provider qwen --secret ".\secrets\aliyun.secret"
```

## 独立复审

对现有成功版本复审时，继续使用工作目录及显式基线；省略 `--section` 会复审整份讲义：

```powershell
uv run video-learner revise ".\output\sample" --base r001 --review --section ch-016 --secret ".\secrets\deepseek.secret"
...
```powershell
uv run video-learner revise ".\output\sample" --base r001 --section ch-001 --instruction "保留关键步骤，压缩重复说明" --secret ".\secrets\deepseek.secret"
uv run video-learner revise ".\output\sample" --base r002 --block blk-001-001 --instruction "修正术语并说明原课中的依据" --secret ".\secrets\deepseek.secret"
```
...
```powershell
uv run video-learner revise ".\output\sample" --base r002 --block fig-001-002 --frame 00:46:12
```
...
默认使用随包保存的[价格快照](../src/video_learner/common/prices.toml)，无需额外开关。该文件集中维护项目采用的估算单价、来源与日期，包含公开报价和用户提供的报价。图文按输入/输出 token 计费，缓存命中输入使用缓存价；ASR 按服务端返回的音频秒数计费。思考 token 已包含在服务返回的输出总量中，不再额外相加；图片同样使用服务计量，不自行按字符数换算。
...
uv run video-learner convert "D:\media\lesson.mp4" --provider qwen --config config.prices.toml --output ".\output\priced-notes"
```
...
转换报告包含本次初稿与自动复审，不包含随后单独修订的费用。`revise --review` 生成独立用量报告；普通文字修订仍保留逐次请求用量日志。
...
音频仍按顺序切分，结果按原切片编号保存；并发不改变切分、时间引用或图文上下文。Qwen ASR 请求（包括重试）共享 `asr_max_calls` 上限；本地识别不使用云端调用预算，CUDA 要求 `jobs=1`。失败或 Ctrl+C 后停止派发和后续重试，等待正在进行的 HTTP 请求返回或超时再保存失败诊断，因此取消可能需要等待 `request_timeout_seconds` 所控制的网络超时。并发不代表任务恢复，也不会增加调用预算。
...
DeepSeek 默认使用配置中的 `deepseek-flash`。默认 `deepseek_context="history"` 将成功章节的原始输入、图片和最终回答完整放在新章节之前；历史只用来承接和统一术语，本章仍只允许引用本次提供的证据。`deepseek_context="chapter"` 可改为独立逐章请求；Qwen 和修订始终使用独立请求。
...
需要模型重新组织时，使用 tools/replay_composition.py 复用章节证据，默认仅离线准备；只有 --live 才付费，真实请求一次最多三章。新生成的工作目录使用调用前保存的证据包，较早产物按现有索引重组并明确提示。复用方法见[性能分析](performance.md#冻结证据回放)。长历史造成引用校验失败时，程序在已有重试预算内隔离为当前章重新请求。
...
第一条核对 r001，第二条核对有冻结记录的文字修订或独立复审。该工具只读本地证据、当次模型输入和已采用结果，检查结构化讲义与生成快照；不请求模型、不覆盖手改、不提供任务恢复。旧产物没有冻结记录时无法进行这种离线重建；精确换图使用原有确定性修订测试验证。
